...
{
  install.packages("dataRetrieval", dependencies = TRUE, INSTALL_opts = '--no-lock')
  install.packages("raster", dependencies = TRUE, INSTALL_opts = '--no-lock')
  install.packages("sf", dependencies = TRUE, INSTALL_opts = '--no-lock')
  install.packages("resample", dependencies = TRUE, INSTALL_opts = '--no-lock')
...
install.packages("wrfhydroSubsetter")
...
old_packages <- installed.packages(lib.loc = "/home/dkim8398/R/x86_64-pc-linux-gnu-library/3.6")
old_packages <- as.data.frame(old_packages)
list.of.packages <- unlist(old_packages$Package)

# remove old packages 
remove.packages( installed.packages( priority = "NA" )[,1], lib="/home/dkim8398/R/x86_64-pc-linux-gnu-library/3.6")
...
install_github("mikejohnson51/AOI")
install_github("donnyDHKim/wrfhydroSubsetter")

library(httr)
TZ="Australia/Sydney" R -e 'devtools::install("/mnt/d/GitHub/wrfhydroSubsetter", repos = NULL, type="source")'

devtools::install("/mnt/d/GitHub/wrfhydroSubsetter", repos = NULL, type="source")
install.packages("/mnt/d/GitHub/wrfhydroSubsetter", 
                 repos = NULL, 
                 type = "source")
library(wrfhydroSubsetter)

# https://rtask.thinkr.fr/installation-of-r-4-0-on-ubuntu-20-04-lts-and-tips-for-spatial-packages/#Delete_previous_version_of_R
