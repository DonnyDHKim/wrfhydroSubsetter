...
subset_wrf_hydro_domain_mod = function(AOI, domain_files, outDir, config){
  ...
  #From namelist.hrldas
  subWrfFile      <- paste0(domain_files, "/wrfinput_.nc")
  subSoilparmFile <- paste0(domain_files, "/soilproperties_", config, ".nc")
  #From hydro.namelist
...
################# SUBSET DOMAIN Files
  ...
  print("fullHydro2dFile processed")
...
if (!file.exists(fullRtlinkFile)) stop(paste0("The fullRtlinkFile : ", fullRtlinkFile, " does not exits"))
    ...
    wrfhydroSubsetter::update_nc(subSpwtFile, subWts[[1]])
    wrfhydroSubsetter::update_nc(subSpwtFile, subWts[[2]])
