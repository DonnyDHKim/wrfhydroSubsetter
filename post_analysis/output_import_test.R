...
## Base Material
#=======================================================
subPath = '/Volumes/Transcend/land-cover-tests/berkeley_west-virginia_5894384_01616500'
geoPath = paste0(subPath + "/geo_em.d0x.nc")

geogrid.raster = wrfhydroSubsetter::make_empty_geogrid_raster(geoPath) #subpath changed to geopath
...
## Channel Routing Material
#=================================
rtlinkFile <- ReadRouteLink(paste0("./Route_Link.nc"))
...
listgages = rtlinkFile$site_no %>% drop_na(mutate(as.numeric())) %>% paste0("0",)

cat("The USGS gages found in the Route_Link.nc file are:", listgages)

#pull out the rows in route_link with gages
gage_comid = arrange(rtlinkFile, desc(site_no))
gage_comid = gage_comid[1:8,]

{
  ## gage location map
  #sbbox <- make_bbox(lon = rtlinkFile$lon, lat = rtlinkFile$lat, f = .1)
  #sq_map <- get_map(location = sbbox, maptype = "satellite", source = "google")  
  #ggmap(sq_map) + 
  #  geom_point(data = gage_comid, mapping = aes(x = lon, y = lat), color = "red") +
  #  geom_text(gage_comid, mapping = aes(label=gage_comid$gages),hjust=0.2, vjust=0, check_overlap = TRUE)
}

### NEED TO MANUALLY CHANGE `gage_USGS` BELOW (these were printed out above)!!!
gage_USGS = '11475560'

# find the USGS gage ID within the Route_Link.nc file
GAGEindex <- which(rtlinkFile$site_no == gage_USGS)

# find the NHDPlus COMID that corresponds to the above USGS gage ID within the Route_Link.nc file
correspondCOMID <- rtlinkFile$link[GAGEindex]
